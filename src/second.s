...
CHECK_THREE_MULTIPLE:
    ...
    je SET_BOKE_FMT                         ; 0ならSET_BOKE_FMTにジャンプする
...
CHECK_IN_THREE:
    ...
    je SET_BOKE_FMT                         ; 余りが0ならSET_BOKE_FMTにジャンプする

    cmp rax, 0                              ; 除算のあまりと0を比較し
    jne CHECK_IN_THREE                      ; 余りが0じゃなければCHECK_IN_THREEにジャンプする

; 普通のフォーマット（数値のみを表示）をrbpに格納する。
SET_BASIC_FMT:
    mov rbp, basic_fmt                      ; rbpに数値のみ表示するフォーマットを格納する

; 次の処理の準備 & `SET_BASIC_FMT`からのジャンプバック用ラベル
READY_TO_SET_FMT_LOOP:
    mov rax, r13                            ; rax(今回表示する値が入ってるレジスタ)にr13（表示用カウントレジスタ）の値をコピー
    mov r15, 14                             ; r15に14（最大このプログラムの耐えられる最大桁数）
...
SET_FMT_LOOP:
    ...
    and rbx, rax                            ; raxと0xFに論理積をかけることで、16進数での1桁分取り出し、rbxに格納
    ...
    jnz SET_FMT_LOOP                        ; raxが0になるまでSET_FMT_LOOPを回し続ける
...
; 次のカウント後の処理へジャンプする
LOOP_BACK:
    cmp r12, 1000001                        ; 最大カウント回数（100000）とr12（通常カウント）を比較
...
; ボケるときのフォーマットをrbpに格納
SET_BOKE_FMT:
    mov rbp, joke_fmt                       ; rbpにボケたときのメッセージフォーマットを格納
    jmp READY_TO_SET_FMT_LOOP               ; `READY_TO_SET_FMT_LOOP`にジャンプする（戻る）
